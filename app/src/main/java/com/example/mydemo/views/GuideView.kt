...
class GuideView : View {
    ...
    //
    private val mGreenArcPaint = Paint()
    //圆弧所在矩形区域
    ...
    private var mRegionList = arrayListOf<Region>()
    ...
    init {
        ...
        mGreenArcPaint.isAntiAlias = true
        mGreenArcPaint.strokeWidth = 1f
        mGreenArcPaint.color = Color.parseColor("#66ff0000")
        mGreenArcPaint.style = Paint.Style.FILL
    }
...
//            canvas.drawPath(path, mArcPaint)
            if(currentIndex == i){
                canvas.drawPath(mPaths[i], mGreenArcPaint)
                when(currentIndex){
                    0 -> guideListener?.onDirection(Direction.RIGHT)
                    1 -> guideListener?.onDirection(Direction.BOTTOM)
                    2 -> guideListener?.onDirection(Direction.LEFT)
                    3 -> guideListener?.onDirection(Direction.TOP)
                }
            } else {
                canvas.drawPath(mPaths[i], mArcPaint)
            }
            ...
            if (i == 0) {//bottom
                canvas.drawBitmap(
                    bitmap,
                    circlePoint.x - bitmap.width / 2,
                    circlePoint.y + mRadius - bitmap.height * 2,
                    mArcPaint
                )
            }
            if (i == 1) {//left
                canvas.drawBitmap(
                    bitmap,
                    circlePoint.x - mRadius + bitmap.width,
                    circlePoint.y - bitmap.height / 2,
                    mArcPaint
                )
            }
            if (i == 2) {//top
                canvas.drawBitmap(
                    bitmap,
                    circlePoint.x - bitmap.width / 2,
                    circlePoint.y - mRadius + bitmap.height,
                    mArcPaint
                )
            }
            if (i == 3) {//right
                canvas.drawBitmap(
                    bitmap,
                    circlePoint.x + mRadius - bitmap.width * 2,
                    circlePoint.y - bitmap.height / 2,
                    mArcPaint
                )
            }
...
//        return super.onTouchEvent(event)
        when(event.action){
            MotionEvent.ACTION_DOWN -> {
                for(i in mRegionList.indices){
                    if(mRegionList[i].contains(event.x.toInt(),event.y.toInt())){
                        currentIndex = i
                        invalidate()
                        break
                    }
    ...
    fun addGuideListener(guideListener:GuideListener){
