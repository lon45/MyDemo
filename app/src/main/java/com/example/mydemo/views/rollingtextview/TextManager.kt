...
internal class TextManager(
    ...
    var showCharBg: Boolean = false

    var textBgColor = Color.GRAY
    init {
        updateFontMatrics()
    }
...
//        }
        ...
        val maxtape = if(targetText.length < sourceText.length) true else false
        ...
        if(maxtape){
            for (idx in 0 until maxLen-targetText.length) {
                textColumns.removeAt(0)
            }
        }
