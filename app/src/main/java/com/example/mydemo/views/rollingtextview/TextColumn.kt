...
import com.example.mydemo.views.rollingtextview.TextManager.Companion.EMPTY
import com.example.mydemo.views.rollingtextview.strategy.Direction
...
internal class TextColumn(
    ...
    fun draw(canvas: Canvas) {
        ...
        if(manager.showCharBg) {
            var bgPaint: Paint = Paint()
            bgPaint.setColor(manager.textBgColor)
//            canvas.drawRect(0f, -textPaint.textSize, textPaint.measureText(changeCharList.get(index).toString()), 0f, bgPaint)

            canvas.drawRoundRect(-5f, -textPaint.textSize, textPaint.measureText(changeCharList.get(index).toString())+4, 10f,4f,4f, bgPaint)
        }
